...
Get2DEffPlots(TTree* t_evt, TString* v1, TString* v2, int xbin, double xmin, double xmax, int ybin, double ymin, double ymax, TCut* cut, TString* cap,bool isPassed){
    TCut* totcut[Ncut];
    totcut[0] = cut;
    ...
    for(int i=0; i<Ncut; i++){
        h2D[i] = new TH2D(Form("h2D_%s_%s_filter%d",v1.Data(),v2.Data(),i), Form("%s;%s;%s",totcut[i]->GetTitle(), v1.Data(), v2.Data()), xbin, xmin, xmax, ybin, ymin, ymax);
    ...
    c_tot->SaveAs(Form("pdf/h2D_%s_isPassed%d_%s_%s_", cap.Data(),(int)isPassed, v1.Data(), v2.Data()));
}
