...
void noiseCompWithDiffTrigger(){
    ...
    for(int i = 0; i < nTrig; ++i){
        ...
        t1->Draw(Form("hf.et>>e%d",i),trig[i]);
        t1->Draw(Form("tower.e>>e%d",i),Form("abs(tower.eta)>2.87 && abs(tower.eta)<5.205 && %s", trig[i]));
        t1->Draw(Form("tower.et>>e%d",i),Form("abs(tower.eta)>2.87 && abs(tower.eta)<5.205 && %s", trig[i]));
    }
    outf->Write();
}
