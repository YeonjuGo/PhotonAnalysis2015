...
//void StarToHepmc(int num=1)
{
  string filename = "slight_jpsi_lhc.out"; //input
  //string filename = "starlight_pbpb_up1s_xn_ee.out"; //input
  ...
  sprintf(outfilename,"starlight_pbpb_jpsi.ee.hepmc-0%d.dat",num);
  ...
  //const unsigned int M =25000; // N_events
  ...
  while (getline(infile,temp_string)) {
    ...
    if(strstr(temp_string.c_str(),"EVENT")) {
        ...
        if(evt_n >=K && evt_n < K+M) output << "V " <<(0-evt_n) << " 0 " << x << " " << y << " " << z << " " << t << " 0 " << N <<  " 0" << endl;
...
void makefiles(int number_of_files=1) {
