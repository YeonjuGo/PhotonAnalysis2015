        ...
        bool isAOD=0)
{
    ...
    const int nECut = 1; // 2, 3 GeV  
    const int nNumTowerCut = 5;//the number of towers for firing HF filter : 1,2,3,4,5 
    const int nMode = 6;
    //Starlight gen, reco, l1 //Hydjet gen, reco, l1
    double frac[nMode][nNumTowerCut] = { {0.0293667,0.014, 0.00695556, 0.00343333,0.00153333},
                                        {},
                                        {},
                                        {},
                                        {},
   ...
   TH1D* h1D[nMode];
    for(int i=0;i<Mode;i++){
        h1D[i] = new TH1D(Form("h1D%d",i),";# of towers for firing HF filter;Fraction", nNumTowerCut, 0,5);
    }
