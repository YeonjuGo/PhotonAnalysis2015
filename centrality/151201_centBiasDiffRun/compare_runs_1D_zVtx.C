...
void draw_compare_runs_1D(string var="zVtx");
void compare_runs_1D_zVtx(){
    TStopwatch timer;
    timer.Start();
    draw_compare_runs_1D("zVtx");//change many things !! 
    draw_compare_runs_1D("vz");//change many things !! 
    timer.Stop();
...
void draw_compare_runs_1D(string var){
    ...
    for(int i=0; i<Nrun; ++i){
        ...
        h[i] = new TH1D(Form("h%d",i), Form(";%s (cm);Event Fraction",var.data()), nBin, -30, 30); // for zVtx 
