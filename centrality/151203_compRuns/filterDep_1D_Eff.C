...
const double dy= 0.5;
//"HLT_HIL1MinimumBiasHF1ANDExpress_v1 && phfCoincFilter3 && pprimaryVertexFilter"
        ...
        bool isAOD=0)
{
    ...
    int nBin = 50;
    //Get1DEffPlots(t_evt, "lumi",111,0,777,trig,type,1,isAOD);
    //Get1DEffPlots(t_evt, "vz",nBin,-25,25,trig,type,1,isAOD);
#if 1
    //Get1DEffPlots(t_evt, "hiHF",nBin,0,5000,trig,type,1,isAOD);
    //Get1DEffPlots(t_evt, "hiNpix",nBin,0,10000,trig,type,1,isAOD);
    Get1DEffPlots(t_evt, "hiBin",100,0,200,trig,type,1,isAOD);
    //Get1DEffPlots(t_evt, "hiNtracks",nBin,0,4000,trig,type,1,isAOD);
#endif
    /*
    //Get1DEffPlots(t_evt, "hiHFhit",nBin,0,4000,trig,type,1,isAOD);
    //Get1DEffPlots(t_evt, "hiZDC",nBin,0,50000,trig,type,1,isAOD);
    Get1DEffPlots(t_evt, "hiET",nBin,0,2000,trig,type,1,isAOD);
    Get1DEffPlots(t_evt, "hiEE",nBin,0,4000,trig,type,1,isAOD);
    Get1DEffPlots(t_evt, "hiEB",nBin,0,5000,trig,type,1,isAOD);
    Get1DEffPlots(t_evt, "hiNtracksPtCut",nBin,0,3000,trig,type,1,isAOD);
    Get1DEffPlots(t_evt, "hiNtracksEtaCut",nBin,0,3000,trig,type,1,isAOD);
    Get1DEffPlots(t_evt, "hiNtracksEtaPtCut",nBin,0,2000,trig,type,1,isAOD);
*/
/*
    nBin=50;
    Get1DEffPlots(t_evt, "hiHF",nBin,0,100,trig,Form("%s_shortRange",type),1,isAOD);
    Get1DEffPlots(t_evt, "hiHFhit",nBin,0,1500,trig,Form("%s_shortRange",type),1,isAOD);
    Get1DEffPlots(t_evt, "hiNpix",nBin,0,300,trig,Form("%s_shortRange",type),1,isAOD);
    Get1DEffPlots(t_evt, "hiBin",nBin,100,200,trig,Form("%s_shortRange",type),1,isAOD);
    Get1DEffPlots(t_evt, "hiNtracks",nBin,0,100,trig,Form("%s_shortRange",type),1,isAOD);
*/
    /*
    //Get1DEffPlots(t_evt, "hiZDC",nBin,0,5000,trig,Form("%s_shortRange",type),1,isAOD);
    Get1DEffPlots(t_evt, "hiET",nBin,0,20,trig,Form("%s_shortRange",type),1,isAOD);
    Get1DEffPlots(t_evt, "hiEE",nBin,0,20,trig,Form("%s_shortRange",type),1,isAOD);
    Get1DEffPlots(t_evt, "hiEB",nBin,0,80,trig,Form("%s_shortRange",type),1,isAOD);
    Get1DEffPlots(t_evt, "hiNtracksPtCut",nBin,0,100,trig,Form("%s_shortRange",type),1,isAOD);
    Get1DEffPlots(t_evt, "hiNtracksEtaCut",nBin,0,100,trig,Form("%s_shortRange",type),1,isAOD);
    Get1DEffPlots(t_evt, "hiNtracksEtaPtCut",nBin,0,50,trig,Form("%s_shortRange",type),1,isAOD);
*/ 
}
...
{
    TCanvas *c_tot = new TCanvas(Form("c_tot_%s_%s",v1.Data(),cap), "c_tot", 400,800);
    c_tot->Divide(1,2);
    ...
    for(int i=0;i<nfilter;i++){
        ...
        if(i!=0){
            c_tot->cd(2);
            h1D_eff[i]->GetYaxis()-> SetRangeUser(h1D_eff[nfilter-1]->GetMinimum()*0.9,1.1);
            if(i==1) {
