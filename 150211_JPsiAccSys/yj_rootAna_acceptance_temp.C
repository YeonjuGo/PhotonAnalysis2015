...
void yj_rootAna_acceptance(char *strBinnig = "8rap9pt", bool isPrompt=true){
    TStopwatch timer;
    timer.Start();   
...
//		ana1->Add("/home/songkyo/kyo/pPbDataSample/AcceptanceSampleKYOvtx/tot_GENonly_inclBtoJPsiMuMu_5TeV02_1st_KYOvtx_ntuple_20150127_p2.root");
  ...
  //Double_t yBinsArr[] = {-2.4, -1.97, -1.37, -0.47, 0.43, 1.03, 1.46}; // 6rap3pt
    ...
    if(isPrompt){
        toyFile_pPb = new TFile("toyResults_pt_number100_isPrompt1_isPbp0.root");
        toyFile_Pbp = new TFile("toyResults_pt_number100_isPrompt1_isPbp1.root");
    } else {
        toyFile_pPb = new TFile("toyResults_pt_number100_isPrompt0_isPbp0.root");
        toyFile_Pbp = new TFile("toyResults_pt_number100_isPrompt0_isPbp1.root");
    }
    
    toyTree_pPb = (TTree*)toyFile_pPb->Get("ditTree");
    toyTree_Pbp = (TTree*)toyFile_Pbp->Get("ditTree");
    AccDef_Pbp = (TH1D*)toyFile_Pbp->Get("hRatio_Pbp");
    AccDef_pPb = (TH1D*)toyFile_pPb->Get("hRatio_pPb");
 
    double par1_pPb, par2_pPb, par3_pPb, par4_pPb; 
    double par1_Pbp, par2_Pbp, par3_Pbp, par4_Pbp; 

    toyTree_pPb->SetBranchAddress("a1",   &par1_pPb);
    toyTree_pPb->SetBranchAddress("a2",   &par2_pPb);
    toyTree_pPb->SetBranchAddress("a3",   &par3_pPb);
    toyTree_pPb->SetBranchAddress("a4",   &par4_pPb);
    toyTree_Pbp->SetBranchAddress("a1",   &par1_Pbp);
    toyTree_Pbp->SetBranchAddress("a2",   &par2_Pbp);
    toyTree_Pbp->SetBranchAddress("a3",   &par3_Pbp);
    toyTree_Pbp->SetBranchAddress("a4",   &par4_Pbp);

    TFile* outFileAcc = new TFile(Form("AccSys_isPrompt%d.root",isPrompt),"recreate");
    ...
    TH1D *h1D_Num_pt_pPb = new TH1D("h1D_Num_pt_pPb","",nPtBins,ptBinsArr);
    TH1D *h1D_Den_pt_pPb = new TH1D("h1D_Den_pt_pPb","",nPtBins,ptBinsArr);
    TH1D *h1D_Acc_pt_pPb = new TH1D("h1D_Acc_pt_pPb","",nPtBins,ptBinsArr);
    TH1D *hAccComp_pPbBin[nPtBins];
    for(int ipt=0;ipt<nPtBins;ipt++){
        double center = AccDef_pPb->GetBinContent(ipt+1);
        hAccComp_pPbBin[ipt] = new TH1D(Form("hAccComp_pPbBin%d",ipt),"",1000, center-0.0500 ,center+0.0500);
        hAccComp_pPbBin[ipt]->Sumw2();
    }
    h1D_Num_pt_pPb->Sumw2();
    h1D_Den_pt_pPb->Sumw2();
    h1D_Acc_pt_pPb->Sumw2();
    ...
    for(int itoy=0; itoy<Ntoy; itoy++){
        ...
        h1D_Num_pt_pPb -> Reset();
        h1D_Den_pt_pPb -> Reset();
        h1D_Acc_pt_pPb -> Reset();
        for(int ipt=0;ipt<nPtBins;ipt++){
            hAccComp_pPbBin[ipt] -> Reset();
        }
        ...
        for(int i=0; i<ana1->GetEntries(); i++){
           // if(i%100000==0) cout << ">>>>> EVENT " << i << " / " << ana1->GetEntries() <<  endl;
            ...
            double w_toy = 1.0;
            if(isPrompt)
                w_toy = par1_pPb/(1+TMath::Exp(par2_pPb*dimu_pt)) + par3_pPb/dimu_pt;
            else
                w_toy = par1_pPb*TMath::Erf((dimu_pt-par2_pPb)/par3_pPb) + par4_pPb;
            ...
            if ( yn == true  ) {
                ...
                h1D_Den_pt_pPb->Fill(dimu_pt,w_toy); //yeonju
                ////// --- cut03 : for numerator
                bool yn2 = false;
                ...
                if (yn2 == true){
                    ...
                    h1D_Num_pt_pPb->Fill(dimu_pt,w_toy);//yeonju
                } // end of yn2
            } // end of yn
        } //end of loop
    } //end of itoy loop
    ...
    h1D_Acc_pt_pPb->Divide(h1D_Num_pt_pPb,h1D_Den_pt_pPb,1,1,"B");
    
    for(int ipt=0;ipt<nPtBins;ipt++){
        hAccComp_pPbBin[ipt]->Fill(h1D_Acc_pt_pPb->GetBinContent(ipt+1));
    }
