...
void phoEnergyScale_eta(TString sample = "AllQCDPhoton30", TString treePath="ggHiNtuplizer", int ptThr=20){
   ...
   TString inputFileName = Form("/afs/cern.ch/work/y/ygo/private/PhotonAnalysis2015/PFphoton/CMSSW_7_5_0/src/150908_voronoiStudy/skimFiles/jskim_%s_%s_genMatched_inclusivePho.root",sample.Data(),treePath.Data());
    ...
    c1->SaveAs(Form("png/ES_eta_ptThr%d_%s.png",ptThr,treePath.Data()));
    c2[0]->SaveAs(Form("png/ES_etaDist_%s_ptThr%d_%s.png",treePath.Data(),ptThr,centSt[0]));
    c2[1]->SaveAs(Form("png/ES_etaDist_%s_ptThr%d_%s.png",treePath.Data(),ptThr,centSt[1]));
}
