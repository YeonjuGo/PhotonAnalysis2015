 ...
 ***********
 modified by Yeonju 
v4 : not using getMatchingEntry() function.
: using ematcher->retrieveEvent(event_file2, lumi_file2, run_file2);
: using CloneTree(0) not SetBranchAddress for all the variables.
: for the photon tree, (not including rechittree)
*/
...
void eventMatcherMacrosYJ_v4()
{
    const char* filePath1= "/mnt/hadoop/cms/store/user/luck/L1Emulator/HiForest_PbPb_photon2030.root";//entry = 349248
    const char* filePath2= "/mnt/hadoop/cms/store/user/tatar/HIHighPt/HiForest_HIHighPt_photon30_HIRun2011-v1.root";
    ...
    //TTree *t_hbhe1 = (TTree*)inputFile1->Get("rechitanalyzer/hbhe");
    TTree *t_pho1 = (TTree*)inputFile1->Get("multiPhotonAnalyzer/photon");
    ...
    //TTree *t_hbhe2 = (TTree*)inputFile2->Get("rechitanalyzer/hbhe");
    TTree *t_pho2 = (TTree*)inputFile2->Get("ggHiNtuplizer/EventTree");
    ...
    const char* outputFileName = "skimFiles/eventMatched_53X_75X_biransSample.root";
    TFile* outputFile = new TFile(outputFileName, "RECREATE");
    TTree *newt_evt = t_evt1->CloneTree(0); 
    //TTree *newt_eb1 = t_eb1->CloneTree(0); 
    //TTree *newt_ee1 = t_ee1->CloneTree(0); 
    //TTree *newt_hbhe1 = t_hbhe1->CloneTree(0); 
    TTree *newt_pho1 = t_pho1->CloneTree(0); 
    //TTree *newt_eb2 = t_eb2->CloneTree(0); 
    //TTree *newt_ee2 = t_ee2->CloneTree(0); 
    //TTree *newt_hbhe2 = t_hbhe2->CloneTree(0); 
    TTree *newt_pho2 = t_pho2->CloneTree(0); 
    newt_evt->SetName("HiTree");
    //newt_eb1->SetName("eb53x");
    //newt_ee1->SetName("ee53x");
    //newt_hbhe1->SetName("hbhe53x");
    newt_pho1->SetName("pho53x");
    //newt_eb2->SetName("eb75x");
    //newt_ee2->SetName("ee75x");
    //newt_hbhe2->SetName("hbhe75x");
    newt_pho2->SetName("pho75x");
    ...
    for(int j=0;j<entries2;++j){
        ...
        //t_hbhe1->GetEntry(matched_entry);
        t_pho1->GetEntry(matched_entry);
