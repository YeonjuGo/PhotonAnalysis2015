...
void isoPhoEff_isoDep(TString sample = "AllQCDPhoton30", TString treePath="ggHiNtuplizer",TString isoType = "sumIsoR", int rad=4, int evpOrder=2)
{
    ...
    for(int ieta=0; ieta<nEtaCut_here; ieta++){
        ...
        for(int icent=0;icent<nCentBin;icent++){
            ...
            for(int j=0;j<nBin;j++){
                double den = tr->GetEntries(Form("mcCalIsoDR0%d<5.0 && %s && %s", rad, etaCut.Data(), centCut.Data()));
                double num = tr->GetEntries(Form("%s>-100 && %s< %.1f && mcCalIsoDR0%d<5.0 && %s && %s",var.Data(), var.Data(), middle_0thBin+j*binWidth, rad, etaCut.Data(), centCut.Data()));
                double eff = num/den;
    ...
    c1->SaveAs(Form("png/isoPhoEff_isoDep_%s_%s_%s_R%d_evpOrder%d_etaTot.png",sample.Data(),treePath.Data(),isoType.Data(),rad,evpOrder));
    c2->SaveAs(Form("png/isoPhoEff_isoDep_%s_%s_%s_R%d_evpOrder%d_etaDiffrential.png",sample.Data(),treePath.Data(),isoType.Data(),rad,evpOrder));
}
