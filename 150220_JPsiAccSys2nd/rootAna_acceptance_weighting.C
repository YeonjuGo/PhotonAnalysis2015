...
void rootAna_acceptance_weighting(bool isPrompt=false, bool isPbp=false){
    ...
    //    cout << ipt << "th ptbin, Acc. = " << h1D_Acc_pt->GetBinContent(ipt+1) << endl;
        for(int iy=0;iy<nYBins;iy++){
            ...
            else if(isPbp==0)originalValue = h2D_Acc_pt_y_Pbp->GetBinContent(iy+1, ipt+1);
            double weightedValue = h2D_Acc_weight_pt_y->GetBinContent(iy+1, ipt+1);
            h2D_Acc_diff_pt_y->SetBinContent(iy+1, ipt+1,TMath::Abs(originalValue-weightedValue)/originalValue);
        }
    }
    ////////////////////////////////////////////////////////////////////////////
    // Save the data!
