...
//
// leading photon condition O, only pthat30 (, and 50 , 80, ...)
#include "TFile.h"
#include "TTree.h"
#include "TNtuple.h"
#include "TH1D.h"
...
void compareDist2(){
	TFile* myFile = new TFile("/u/user/goyeonju/2014/141015_JEC/CMSSW_5_3_20/src/JetMETAnalysis/JetAnalyzers/bin/test/pPb30to9999.root");
	TFile* alexFile = new TFile("../test/MIX_pPb30to9999.root");
//	TFile* myFile = new TFile("/u/user/goyeonju/2014/141015_JEC/CMSSW_5_3_20/src/JetMETAnalysis/JetAnalyzers/bin/test/pPb50to9999.root");
//	TFile* alexFile = new TFile("../test/MIX_pPb50to9999.root");
//	TFile* myFile = new TFile("/u/user/goyeonju/2014/141015_JEC/CMSSW_5_3_20/src/JetMETAnalysis/JetAnalyzers/bin/test/pPb80to9999.root");
//      TFile* alexFile = new TFile("../test/MIX_pPb80to9999.root");
//	TFile* myFile = new TFile("/u/user/goyeonju/2014/141015_JEC/CMSSW_5_3_20/src/JetMETAnalysis/JetAnalyzers/bin/test/pPb120to9999.root");
//	TFile* alexFile = new TFile("../test/MIX_pPb120to9999.root");
...
//======================================
	...
	myTree -> Draw("pt>>+mypt", "(pt<1000)");
	mypt = (TH1D*)gDirectory->Get("mypt");
	alexTree -> Draw("pt>>+alexpt", "(pt<1000)");
	alexpt = (TH1D*)gDirectory->Get("alexpt");

	float myptArea = mypt->Integral("width");
	float alexptArea = alexpt->Integral("width");
	mypt -> Scale(1./myptArea);
	alexpt -> Scale(1./alexptArea);
...
//	myjtpt -> Scale(1./myptArea);
//	alexjtpt -> Scale(1./alexptArea);
...
//======================================
	...
	//myrefpt -> Scale(1./myptArea);
	//alexrefpt -> Scale(1./alexptArea);
