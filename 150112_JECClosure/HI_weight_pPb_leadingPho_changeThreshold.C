// Author : Yeonju Go
// leading photon conditions & cut gen photon threshold from 25 to 35 GeV
...
void HI_weight_pPb_leadingPho_changeThreshold(const char *infile="/mnt/hadoop/cms/store/user/belt/Validations53X/Track8_Jet29_cut1/QCDpT15_2011RECO_STARTHI53_LV1_5_3_16_Track8_Jet29_1GeVcut_badJEC_forest.root",
		...
		)
{
	...
	tr_skim->SetBranchStatus("pPAprimaryVertexFilter",1,0);
        ...
        Float_t ptgen[1000], etagen[1000], phigen[1000];
        Int_t mult, pdg[1000];
	TTree *tr_gen = 0;
        ...
        tr_gen->SetBranchAddress("phi", phigen);
	...
	tr_gen->SetBranchStatus("phi",1,0);
	...
	//	tr_out->Branch("pprimaryVertexFilter",&pprimaryVertexFilter,"pprimaryVertexFilter/I");
		...
		for (Long64_t i=0; i<fentries;i++) 
		{
			...
			//! gen loop 
			Float_t phoPtThr = 25; // photon threshold is 35 
			Int_t nAboveThr = 0;
			Int_t nbw25to35 = 0;
			for(Int_t igen=0; igen<mult; igen++)
			{
				...
				if(ptgen[igen]>=25 && ptgen[igen]<35) nbw25to35 += 1;
				if(ptgen[igen]<phoPtThr) continue;
				nAboveThr+=1;
			}
			
			//cout << nAboveThr << "   " << nbw25to35<< endl;
