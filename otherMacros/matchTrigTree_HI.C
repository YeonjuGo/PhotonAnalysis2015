...
int matchTrigTree_HI(const std::string inHLTFile, const std::string inForestFile, const std::string inTrigFileName, const std::string inOptMaxName = "")
{
    ...
    */
	...
	for(Int_t iter2 = 0; iter2 < trigTypeCount[iter]; iter2++){
	  ...
	  */
      ...
      if(TMath::Abs(maxElectronAnaEta) < 1.44 && TMath::Abs(twoElectronAnaEta) < 1.44 && maxElectronAnaPt > 15 && twoElectronAnaPt > 15 && trueZMass > 60 && trueZMass < 120/* && hiBin < 20*/){
	for(Int_t iter2 = 0; iter2 < trigTypeCount[iter]; iter2++){
	  nZTrigFireDenom[iter][iter2]++;	  

	  if(trigVal[iter][iter2] || trigVal[14][4] || trigVal[17][5]) nZTrigFireNum[iter][iter2]++;
	}
      }
  ...
  std::cout << std::endl;
  std::cout << "trigName: " << trigName[14][4] << ", " <<  trigName[17][5]  << std::endl;
