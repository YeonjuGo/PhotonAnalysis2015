...
int plotTrigTurnOn_HI(const std::string inHistFile, const std::string inTrigFileName, const std::string inOptionalEffFile = "")
{
  ...
  for(Int_t iter = 0; iter < nTrigType; iter++){      
    ...
    for(Int_t iter2 = 0; iter2 < trigTypeCount[iter]; iter2++){
      ...
      if(iter2 == 0){
	TH1F* hEmpty = new TH1F("hEmpty", ";p_{T}^{reco};Efficiency", aPt_p[iter][iter2]->GetN(), 0/*aPt_p[iter][iter2]->GetXaxis()->GetXmin()*/, aPt_p[iter][iter2]->GetXaxis()->GetXmax());
  ...
  for(Int_t iter = 0; iter < nTrigPlotType; iter++){
    ...
    rateLeg_p[iter] = new TLegend(0.68, 0.70, 0.98, 0.90);
  ...
  for(Int_t iter = 0; iter < nTrigType; iter++){
    ...
    for(Int_t binIter = 0; binIter < ratesUnmatched_p[iter]->GetNbinsX(); binIter++){
      Double_t binVal = ratesUnmatched_p[iter]->GetBinContent(binIter+1);

      if(maxYRate[trigPlotPos[iter]] < binVal) maxYRate[trigPlotPos[iter]] = binVal;
      if(minYRate[trigPlotPos[iter]] > binVal && binVal!= 0) minYRate[trigPlotPos[iter]] = binVal;
    }
  ...
  for(Int_t iter = 0; iter < nTrigPlotType; iter++){
    ...
    maxYRate[iter] += 2*TMath::Sqrt(maxYRate[iter]);

    std::cout << trigType[iter] << ", " << minYRate[iter] << std::endl;
    minYRate[iter] /= 5;
    if(minYRate[iter] < .000000001) minYRate[iter] = 0.000000001;
    ...
    hEmptyRate[iter] = new TH1F(Form("hEmptyRate%d", iter), Form(";p_{T,%s}^{trig};Rate (Hz) 1500 bunches+1.7PU", trigPlotType2[iter].c_str()), 10, minXRate[iter], maxXRate[iter]);
