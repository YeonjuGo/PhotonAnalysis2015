...
void makeSkimGenMatching_HiGenTree(const char* hiForestfileName="/afs/cern.ch/work/y/ygo/private/PRODUCTION/CMSSW_7_5_0/src/PFphoton/nominalForest/crab_AllQCDPhoton30/results/merged_AllQCDPhoton30_PbPb_5TeV_forest_pfisoAdded_genThr500MeV_2nd.root",
        TString outName="AllQCDPhoton30", TString treePath="ggHiNtuplizer", float ptThr=20)
{
    ...
    for(Long64_t jj = 0; jj < entries; ++jj)
    {
        ...
        for (int i=0; i < nPho; ++i)
        {
            ...
            for (int j=0; j<mult; ++j){
                ...
                if( pt->at(j) <= 5 ) continue;
                //if( ! (mcPt->at(j)/phoEt->at(i) >=0.5 && mcPt->at(j)/phoEt->at(i)<=1.5) ) continue;
                double deltaRtmp = getDR(phoEta->at(i), phoPhi->at(i), eta->at(j), phi->at(j));
...
*/
            passedPromptPho=1;
            if(passedPromptPho){
                mcPt_->push_back(pt->at(matchedIndex));
                mcPhi_->push_back(phi->at(matchedIndex));
                mcEta_->push_back(eta->at(matchedIndex));
                mcPID_->push_back(pdg->at(matchedIndex));
                mcDPhi_evtpl1_->push_back(getDPHI(phi->at(matchedIndex),hiEvtPlanes[2]));
                mcDPhi_evtpl2_->push_back(getDPHI(phi->at(matchedIndex),hiEvtPlanes[8]));
                mcDPhi_evtpl3_->push_back(getDPHI(phi->at(matchedIndex),hiEvtPlanes[15]));
                mcDPhi_evtpl4_->push_back(getDPHI(phi->at(matchedIndex),hiEvtPlanes[21]));
