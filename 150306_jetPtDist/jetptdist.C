...
//	TCut kineCut = "abs(eta)<1.6 && dphi>7*3.1415/8. && abs(photonEta)<1.44";
	...
	totCut_data[0] = kineCut;
	totCut_data[1] = kineCut && sigmaCut;
	totCut_data[2] = kineCut && sigmaCut && isoCut;
	TCut totCut_mc[nTotCut];
//	totCut_mc[0] = kineCut && mcCut;
//	totCut_mc[1] = kineCut && sigmaCut && mcCut;
	totCut_mc[0] = kineCut;
	totCut_mc[1] = kineCut && sigmaCut;
	totCut_mc[2] = kineCut && sigmaCut && isoCut && mcCut;
...
//	c_cut -> Update();
	c_cut ->SaveAs("pdf/jetptdist.pdf");
}
