...
void jetptdist_evtSizeDep(bool isCentral=1){
	TH1::SetDefaultSumw2();
	// gStyle->SetOptFit(0);
...
//	const double ptbins[] = {20,30,40,50,60,70,80,90,100,110,120,130,140,150,160,170,180,190,200};
	...
	const int nPhoCut = 3;
        ...
        TCut centCut = "";
        if(isCentral) centCut = "30<=hf4Sum && hf4Sum<=120";
        else centCut = "0<=hf4Sum && hf4Sum<=20";
	TCut kineCut = "abs(eta)<1.6 && dphi>7*3.1415/8. && abs(photonEta)<1.44";
//	TCut kineCut = "abs(eta)<1.6 && dphi>2*3.1415/3. && abs(photonEta)<1.44";
	...
	totCut_data[0] = kineCut && centCut;
	totCut_data[1] = kineCut && sigmaCut && centCut;
	totCut_data[2] = kineCut && sigmaCut && isoCut && centCut;
	TCut totCut_mc[nTotCut];
//	totCut_mc[0] = kineCut && mcCut;
//	totCut_mc[1] = kineCut && sigmaCut && mcCut;
	totCut_mc[0] = kineCut && centCut;
	totCut_mc[1] = kineCut && sigmaCut && centCut;
	totCut_mc[2] = kineCut && sigmaCut && isoCut && mcCut && centCut;
	...
	for(int i=0;i<nTotCut;i++){
		for(int j=0;j<nPhoCut;j++){
			hjetpt_data[i][j] = new TH1D(Form("hjetpt_data_tot%d_pho%d",i,j), Form("hjetpt_data_tot%d_pho%d;p_{T}^{Jet};",i,j), nptbins, ptbins);
			hjetpt_mc[i][j] = new TH1D(Form("hjetpt_mc_tot%d_pho%d",i,j),  Form("hjetpt_mc_tot%d_pho%d;p_{T}^{Jet};",i,j), nptbins, ptbins);
			hpho_data[i][j] = new TH1D(Form("hpho_data_tot%d_pho%d",i,j),  Form("hpho_mc_tot%d_pho%d;p_{T}^{#gamma};",i,j), nptbins, ptbins);
			hpho_mc[i][j] = new TH1D(Form("hpho_mc_tot%d_pho%d",i,j), Form("hpho_mc_tot%d_pho%d;p_{T}^{#gamma};",i,j), nptbins, ptbins);
		}	
	}
...
//	c_cut -> Update();
	c_cut ->SaveAs(Form("pdf/jetptdist_isCentral%d.pdf",isCentral));
}
